...
function FastBuy() {
  ...
  const { user, _getBaseAssetAvailableBalance, baseAvailableBalance } = useContext(ApiContext);
  ...
  function balancePercentageHandler(percentage) {
    const price = parseFloat(choosed?.lastPrice);
    const amount = getQuantityWithRatioToBuy(percentage, baseAvailableBalance, price);
    ...
    <>
      ...
      <Row>
        <Col md={12}>
          <span className="exchange-balance-info">
            {isLoggedIn ? `My ${quote} balance: ${baseAvailableBalance}` : `My ${quote} balance: 0`}
          </span>
        </Col>

        {/*<Col md={12}>*/}
        {/*  <span className="exchange-balance-info">{isLoggedIn ? `Obtainable: ${baseAvailableBalance} $` : `Obtainable: 0 $`}</span>*/}
        {/*</Col>*/}
      ...
      <Row>
        <Col md={12} className="mt-3">
          <div className="exchange-total py-3">
      ...
      <Row>
        <Col md={12} className="my-4">
          {isLoggedIn ? (
            <button
              onClick={() => console.log("BUY ACTION HERE")}
              type="button"
              className="btn buy"
