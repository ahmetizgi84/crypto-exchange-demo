...
Http.interceptors.request.use(
  function (config) {
    ...
    if (preferredLang) config.headers['Accept-Language'] = preferredLang.code;

    var selectedTenant = cacheGet('selectedTenant');
    if (selectedTenant) config.headers['X-TenantId'] = selectedTenant.tenantId;
...
Http.interceptors.response.use(
  function (response) {
    ...
    if (_.get(data, 'data.isWorkflowStarted', false)) return Promise.reject('WORKFLOW_DELEGATED');
    else if (_.get(data, 'data.isWorkflowFaulted', false)) return Promise.reject('WORKFLOW_FAULTED');
    else if (_.get(data, 'data.isWorkflowFinished', false)) return Promise.reject('WORKFLOW_FINISHED');
    else if (
